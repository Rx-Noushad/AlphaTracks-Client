...
const useFirebase = () => {
    ...
    useEffect(() => {
        fetch(`https://alpha-tracks-server-site-kw9aq2sdn-rx-noushad.vercel.app/users/${user.email}`)
            .then(res => res.json())
            .then(data => {
                setAdmin(data.admin);
            })
    ...
    const saveUser = (displayName, email) => {
        ...
        console.log(user);
        fetch('https://alpha-tracks-server-site-kw9aq2sdn-rx-noushad.vercel.app/users', {
            method: 'POST',
            headers: {
